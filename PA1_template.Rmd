...
filled.activity.patt  <- with(data = filled.activity, tapply(X = steps, INDEX = date, FUN = sum))
filled.activity.patt  <- data.frame(date = names(filled.activity.patt), tot.steps = filled.activity.patt)
hist(filled.activity.patt$tot.steps, xlab = "Total Steps", 
            main = "Histogram of total number \n of steps taken each day(null values amputed)")
            
print(paste("Mean (with only clean values):", mean(clean.values$steps)))         
print(paste("Mean (with filled values):", mean(filled.activity$steps)))

print(paste("Median (with only clean values):", median(clean.values$steps)))         
print(paste("Median (with filled values):", median(filled.activity$steps)))
